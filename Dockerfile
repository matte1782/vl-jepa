...
HEALTHCHECK --interval=30s --timeout=10s --start-period=60s --retries=3 \
    CMD python -c "import os; import urllib.request; urllib.request.urlopen(f'http://localhost:{os.environ.get(\"PORT\", 8000)}/api/health')" || exit 1

# Default command: Launch FastAPI server
# Uses shell form to expand PORT env variable (Render provides its own PORT)
CMD ["sh", "-c", "uvicorn vl_jepa.api.main:app --host 0.0.0.0 --port ${PORT:-8000}"]
